...
const newString = strings.filter((str) => {
  return str.length > 5
}) 
...
const concatenate = alphabets.reduce((previousChar, currentChar)=>{
...
const firstEven = number.find((number)=>{
  return number%2 === 0
...
const index = ranks.findIndex((number)=>{
  return number === 7
})
console.log(index)
...
const lessThen45 = marks.some((number)=>{
  return number<45;
})
console.log(lessThen45)
...
const checkPositive = positive.every((number)=>{
  return number>0;
})
...
cars.forEach((car,index)=>{
  if(index===0){
    ...
    console.log(`${car.name} ${car.color[0]}`)}

  else if(index === 1){
    console.log(`${car.name} ${car.color[2]} ${car.capacity}`)
  }

  else if (index===3){
    console.log(`${car.name} ${car.color[1]} ${car.capacity}`)
  }
...
const checkPropertyBike = person.vehicles.hasOwnProperty("bike")
const checkPropertyHouse = person.vehicles.hasOwnProperty("house")
console.log(checkPropertyBike)
console.log(checkPropertyHouse)
